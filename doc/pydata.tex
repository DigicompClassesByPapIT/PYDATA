...
\begin{document}
    ...
    \begin{frame}{La Data Science~?}{Un nouveau concept}
        \centering
        \includegraphics[width=11cm]{image/stackoverflow-data-science-trends-chart}
    \end{frame}

    \begin{frame}{Pourquoi la Data Science en Python~?}{Un langage très populaire\footnote{\label{devecosystem-2024}Which programming languages have you used in the last 12 months~?, \url{https://www.jetbrains.com/lp/devecosystem-2024/}}}
        \centering
        \includegraphics[width=7.5cm]{image/all-languages}
    \end{frame}

    \begin{frame}{Pourquoi la Data Science en Python~?}{Un langage très populaire dans la data science}
        Les langages les plus populaires dans la data science sont en général dans l'ordre suivant~:
        \begin{itemize}
            \item Python
            \item SQL
            \item R
        ...
        \begin{dangercolorbox}
            Pas de donnée chiffrée provenant d'un blog/service ou enquête.
        \end{dangercolorbox}
    \end{frame}
    ...
    \begin{frame}{Quelles librairies Python choisir~?}{Pour le traitement de données\footnote{\label{the-state-of-data-science-2024}The State of Data Science 2024: 6 Key Data Science Trends, \url{https://blog.jetbrains.com/pycharm/2024/12/the-state-of-data-science/}}}
        \centering
        \includegraphics[width=8cm]{image/data-wrangling-framework}
    \end{frame}

    \begin{frame}{Quelles librairies Python choisir~?}{Pour la \textit{dataviz}\cref{the-state-of-data-science-2024}}
        \centering
        \includegraphics[width=10cm]{image/dataviz-framework}
    \end{frame}

    \begin{frame}{Quelles librairies Python choisir~?}{Pour le Machine Learning (ML)\cref{the-state-of-data-science-2024}}
        \centering
        \includegraphics[width=9cm]{image/ml-framework}
    \end{frame}

    \begin{frame}{Où lister ces modules (dépendances/librairies)~?}{Installer des packages distants}
    ...
    \begin{frame}{Jupyter Notebook}
        ...
        Par défaut, ce document est un fichier \lstinline{.ipynb} (IPython Notebook) qui peut être ouvert et exécuté dans un navigateur web.
        Les données qu'il contient sont au format JSON, ses versions peuvent donc être suivies dans un VCS comme Git.
        Et même affichées sur les serveurs comme GitHub.
        \bigbreak
        Il est très utilisé en Data Science, \textit{etc} pour son interactivité et sa facilité d'utilisation.
        Par opposition à d'autre domaine de la programmation Python comme le web, le testing qui utilisent un IDE
    \end{frame}
    ...
    \begin{frame}[fragile]{Jupyter Notebook}
        ...
        \bigbreak
        Toutes ces extensions (Toree, IPyWidgets, \textit{etc}) sont également des packages Python qui peuvent être installés avec \lstinline{pip} ou listés dans le \lstinline{requirements.txt}.
    \end{frame}

    \subsection{Fonctionnement local, serveur, dans l'IDE}
    \begin{frame}{Jupyter Notebook}{Notebook dans l'IDE, une aide précieuse}
        \begin{center}
            \includegraphics[width=12cm]{image/pycharm-notebook-repl}
        \end{center}
    \end{frame}
    ...
    \begin{frame}{Jupyter Notebook}{Notebook dans l'IDE, une aide précieuse}
        ...
        \begin{itemize}
            ...
            Un virtual environnement est un environnement Python isolé, qui permet de ne pas polluer l'environnement global de Python et éviter les conflits de version.
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Jupyter Notebook}{Interface et commandes\footnote{Jupyter Notebook tips and shortcuts, \url{https://digitalhumanities.hkust.edu.hk/tutorials/jupyter-notebook-tips-and-shortcuts/}}}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \footnotesize
                \item \lstinline{!} en début d'une ligne d'une cellule permet d'ajouter une commande Shell, sorting du Shell Python.
                \item \textbf{\texttt{Ctrl + Enter}~: Exécuter la cellule}
                \item \textbf{\texttt{Shift + Enter}~: Exécuter et passer à la suivante}
            \end{itemize}
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}[fragile]{Jupyter Notebook}{Comment coder dans un Notebook~?}
        \small
        \begin{dangercolorbox}
            Appliquer les bonnes pratiques de développement logiciel et Python (PEP).
            \textbf{Être dans la data science n'excuse rien~!}

            Si les données, les data visualizations, sont validées par l'équipe, il est important de pouvoir les extraire tel quelles du Notebook pour ne pas recoder et introduire un bug.
...
voila --TagRemovePreprocessor.remove_cell_tags='{"hide"}' mon_notebook.ipynb
    ...
    \subsection{Expressions \textit{lambda}}
    \begin{frame}[fragile]{Expressions\textit{lambda}}{Map, filter et reduce}
        \small
        Il existe de nombreux langages fonctionnels~: Haskell, Lisp, Erlang, Scala, F\#.
        \bigbreak
        D'autres langages supportent la programmation fonctionnelle comme Python, JS, Java entre autres et ont des outils/librairies dédiées à la programmation fonctionnelle.
        \bigbreak
        ...
        \begin{itemize}
            \item Les fonctions pures, qui ne modifient pas, ni ne dépendent pas de l'état du programme ou de son environnement. Elles sont à favoriser.
            \item Les fonctions impures ou avec effet de bord, qui modifient l'état du programme ou de la machine (affichage) ou dépendent de l'état du programme, d'une condition extérieure\ldots{} Elles sont à éviter.
        \end{itemize}
        Les fonctions \textit{lambda} venant du \textit{lambda calculus}.
        Ces expressions sont souvent associées à des \textit{High Order Function} (HOF) map, reduce, filter, etc.
    \end{frame}

    \begin{frame}{Expressions\textit{lambda}}{Pourquoi des fonctions et non des objets}
        Scala prétend que la programmation fonctionnelle permet une mise à l'échelle plus simple des gros projets.
        D'où son nom, Scala pour \textit{SCAlable LAnguage}.

        Mais Pourquoi~?
        \bigbreak
        \begin{itemize}
            \item Les attributs des objets (OOP) peuvent être modifiés à divers endroits du code, ce qui rend le code difficile à comprendre et à maintenir.
            Ce système n'est pas \textit{stateless}.
            \item Les fonctions pures n'ont pas d'état, elles ne modifient pas l'état du programme.
            Elles sont donc \textit{stateless} et seraient plus simples à comprendre et à maintenir.
        \end{itemize}
        \begin{dangercolorbox}
    ...
    \begin{frame}[fragile]{Expressions\textit{lambda}}{Map, Reduce et Filter}
        \begin{footnotesize}
...
24
    ...
    \begin{frame}[fragile]{List comprehension}
        ...
        \bigbreak
        Exemple d'une fonction qui nettoie du texte en ne gardant que les mots de plus de 3 caractères et uniformise la case~:
        \begin{lstlisting}[language=bash,basicstyle=\tiny\ttfamily]
...
Out[6]: array(['1', '2', '3.3'], dtype='<U21')
        ...
        Il est souvent utilisé derrière d'autres librairies comme Pandas, Matplotlib et Scikit-learn.
        \bigbreak
    ...
    \begin{frame}{NumPy}{Fonctions et objets\cref{numpy-website}}
        \begin{itemize}
            \item Les tableaux sont créés directement sur le module \lstinline{numpy} ou \lstinline{np} si importé avec \lstinline{import numpy as np}.
            \item Les fonctions mathématiques de base sont dans \lstinline{numpy.emath}.
            \item Les types sont dans \lstinline{numpy.dtypes}.
        \end{itemize}
        \bigbreak
        \centering
        \includegraphics[width=7cm]{image/numpy-humor}
    \end{frame}
    ...
    \begin{frame}{NumPy}{La pratique}
        ...
        \bigbreak
        \centering
        \includegraphics[width=5cm]{image/digicomp-lightbulb}
    \end{frame}
    ...
    \begin{frame}{Pourquoi Pandas en plus de NumPy et PyArrow~?}
        \begin{itemize}
            \item Numpy est adapté aux arrays et non aux données tabulaires
        ...
        \begin{dangercolorbox}
            ...
            Il est important que les API des outils à sa disposition soient claires\ldots
        \end{dangercolorbox}
    \end{frame}
    ...
    \begin{frame}[fragile]{Structures de données (Series, DataFrame)}
        ...
        Une \lstinline{Series} est l'équivalent d'une colonne dans un \lstinline{DataFrame}.
        \begin{lstlisting}[language=bash]
In [46]: import pandas as pd
In [47]: car = pd.DataFrame({
             "model": ["Mercedes", "BMW", "Dacia"],
             "price": [41_000, 37_000, 15_000]
         })
...
2     Dacia  15000
    ...
    \begin{frame}[fragile]{Structures de données (Series, DataFrame)}
        On peut sélectionner une \lstinline{Series}~:
        \begin{lstlisting}[language=bash]
In [3]: car = pd.DataFrame({
             "model": ["Mercedes", "BMW", "Dacia"],
             "price": [41_000, 37_000, 15_000],
             "year": [2020, 2021, 2019]
         })
...
Out[4]: pandas.core.series.Series
        \end{lstlisting}
        \bigbreak
        On peut également sélectionner une ou plusieurs séries dans un \lstinline{DataFrame} et donc avoir un nouveau \lstinline{DataFrame} en retour~:
        \begin{lstlisting}[language=bash]
In [5]: type(car[["model", "year"]]) # 2 square braquet donc un Dataframe
Out[5]: pandas.core.frame.DataFrame
In [6]: type(car[["price"]]) # 2 square braquet pas une seule
Out[6]: pandas.core.series.DataFrame
        \end{lstlisting}
    \end{frame}
    ...
    \begin{frame}{Excel, CSV, XML, JSON, Parquet, \ldots}
        Selon la documentation de Pandas, cette librairie peut lire de nombreux types de fichiers pour en retourner des \lstinline{pandas.DataFrame}~:
        \begin{columns}
            \column{0.5\textwidth}
            \begin{itemize}
...
df_7965 = df_orders[df_orders["id_store"] == 7965]
        \end{lstlisting}
        \begin{dangercolorbox}
            Appliquer le masque ne modifie pas le DataFrame, il permet d'en retourner un nouveau qu'il faut assigner à une nouvelle variable.

            Il en va de même pour la plupart des transformations de DataFrame, elles ne sont pas \textit{in place}, il faut penser à stocker le retour.
            Certaines exceptions cependant, comme \lstinline{pandas.DataFrame.drop}, qui modifie le DataFrame sur place si l'on change la valeur de l'argument \lstinline{inplace} à \lstinline{True}.
        \end{dangercolorbox}
    \end{frame}
    ...
    \begin{frame}[fragile]{Traitement des données manquantes}
        ...
        \bigbreak
        Plusieurs options sont possibles pour le traitement ce manque de données.
        \begin{itemize}
...
assert len(df_sub_filtered["id_store"].notnull()) == len(df_sub[df_sub_mask])
    ...
    \begin{frame}{Traitement des données manquantes}
        \centering
        \includegraphics[width=8cm]{image/pandas-isnull}
        \bigbreak
        \includegraphics[width=8cm]{image/pandas-notnull}
    \end{frame}
    ...
    \begin{frame}{YData Profiling}
        ...
        \begin{itemize}
            ...
            \item de visualiser les distributions des variables, comme un \href{https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.describe.html}{\lstinline{pandas.DataFrame.describe}}
            \item de visualiser les corrélations entre les variables
            \item de visualiser les valeurs aberrantes (\textit{outliers})
            \item \textit{Many more}\ldots{}
        \end{itemize}
    ...
    \begin{frame}{Combinaison et transformation de données}{Merge de Dataframes}
        \centering
        \includegraphics[width=9cm]{image/dataframe-merge} \\ Merge de DataFrames
    \end{frame}

    \begin{frame}[fragile]{Combinaison et transformation de données}{Merge de Dataframes}
        Il est possible de combiner plusieurs \lstinline{pandas.DataFrame} en un seul.

        En fusions deux DataFrames sur une colonne qu'ils ont en commun, ici \lstinline{model}, avec la méthode \href{https://pandas.pydata.org/docs/reference/api/pandas.merge.html}{\lstinline{pandas.merge}}.
        \begin{lstlisting}[language=python]
import pandas as pd
df_car = pd.DataFrame({"model": ["Mercedes", "BMW", "Dacia"], "price": [41_000, 37_000, 15_000]})
df_production_years = pd.DataFrame({"model": ["Mercedes", "BMW", "Dacia"], "year": [2020, 2021, 2019]})
df_car = pd.merge(df_car, df_production_years, on="model", how="inner") # SQL style
        \end{lstlisting}
        \lstinline{df_car} contient les deux DataFrame~:
        \begin{lstlisting}[language=bash]
In [5]: df_car.head(2)
Out[5]:
      model  price  year
0  Mercedes  41000  2020
1       BMW  37000  2021
        \end{lstlisting}
    \end{frame}

    \begin{frame}{Combinaison et transformation de données}{Concatenation de Dataframes}
        \centering
        \includegraphics[width=8cm]{image/dataframe-concatenation} \\ Concatenation de DataFrames
    \end{frame}

    \begin{frame}[fragile]{Combinaison et transformation de données}{Concatenation de Dataframes}
        En concaténant deux DataFrames \textbf{de même format}, avec la méthode \href{https://pandas.pydata.org/docs/reference/api/pandas.concat.html}{\lstinline{pandas.concat}}.
        \begin{lstlisting}[language=python]
import pandas as pd
df_cars = pd.DataFrame({"model": ["Mercedes", "BMW", "Dacia"], "price": [41_000, 37_000, 15_000]})
df_cheaper_cars = pd.DataFrame({"model": ["Nissan", "Fiat"], "price": [15_000, 10_000]})
df_cars = pd.concat([df_cars, df_cheaper_cars], ignore_index=True)
        \end{lstlisting}
        \lstinline{df_cars} contient maintenant les deux DataFrame~:
        \begin{lstlisting}[language=bash]
In [8]: df_cars.head(5)
Out[8]:
      model  price
...
4      Fiat  10000
    ...
    \begin{frame}{Agrégation et regroupement de données}
        S'il y a une colonne en trop, par exemple le detail d'une facture, alors que l'on veut uniquement les factures.
        On peut obtenir une seule ligne par facture en faisant un \lstinline{pandas.DataFrame.groupby} et une somme sur la liste de chaque produit.
        \bigbreak
        Le \lstinline{pandas.DataFrame.groupby} agrège et doit être associé à une fonction d'agrégation comme~:
        \begin{itemize}
            \item \href{https://pandas.pydata.org/docs/reference/api/pandas.core.groupby.DataFrameGroupBy.mean.html\#pandas.core.groupby.DataFrameGroupBy.mean}{\lstinline{pandas.core.groupby.DataFrameGroupBy.mean}}~: pour la moyenne
            \item \href{https://pandas.pydata.org/docs/reference/api/pandas.core.groupby.DataFrameGroupBy.median.html\#pandas.core.groupby.DataFrameGroupBy.median}{\lstinline{pandas.core.groupby.DataFrameGroupBy.median}}~: pour la médiane
            \item \href{https://pandas.pydata.org/docs/reference/api/pandas.core.groupby.DataFrameGroupBy.sum.html\#pandas.core.groupby.DataFrameGroupBy.sum}{\lstinline{pandas.core.groupby.DataFrameGroupBy.sum}}~: pour la somme
            \item \href{https://pandas.pydata.org/docs/reference/api/pandas.core.groupby.DataFrameGroupBy.count.html\#pandas.core.groupby.DataFrameGroupBy.count}{\lstinline{pandas.core.groupby.DataFrameGroupBy.count}}~: pour énumérer
            \item \href{https://pandas.pydata.org/docs/reference/api/pandas.core.groupby.DataFrameGroupBy.agg.html\#pandas.core.groupby.DataFrameGroupBy.agg}{\lstinline{pandas.core.groupby.DataFrameGroupBy.agg}}~: pour passer sa propre fonction d'agrégation en argument
        \end{itemize}
    \end{frame}

    \subsection{Traitement des données temporelles}

    \begin{frame}[fragile]{Traitement des données temporelles\footnote{How to handle time series data with ease~?, \url{https://pandas.pydata.org/pandas-docs/version/1.2.4/getting_started/intro_tutorials/09_timeseries.html}}}
        Pour accéder aux attributs et méthodes d'une colonne, \textit{i.e.}, \lstinline{pandas.Series} il faut s'assurer que le type est bien un type temporel.
        Pour caster une \lstinline{pandas.Series}, il existe la méthode \lstinline{pandas.to_datetime} qui permet de convertir une colonne en type temporel \lstinline{datetime}.
        \bigbreak
        \begin{lstlisting}[language=python]
import pandas as pd
...
air_quality.groupby(
    [air_quality["datetime"].dt.weekday, "location"]
        ...
        \pause
        \bigbreak
        Cette transformation permet de créer une nouvelle donnée, le jour de la semaine et d'analyser la qualité de l'air en fonction de cette dernière.
    \end{frame}
...
np.array([1, 2, 3, True])
Out[6]: array([1, 2, 3, 1]) # Casting abrupte!!!
np.array([1, 2, 3, "4"])
...
memory usage: 180.0+ bytes
    ...
    \begin{frame}[fragile]{Les types en Numpy et Pandas}{Pandas}
        ...
        Inutiles ici, car un entier de 16 bits non signé va jusqu'à 65 535, attention avec une Ferrari\ldots
        \begin{lstlisting}[language=bash,basicstyle=\tiny\ttfamily]
...
Out[45]: numpy.uint16
    ...
    \begin{frame}{Plotly}{Plotly et Plotly Express}
        \footnotesize
        Plotly est une librairie de visualisation de données qui affiche des graphiques interactifs de différents formats~:
        \begin{itemize}
            \item Graphique HTML à insérer dans une page web
            \item Graphique PNG, SVG, PDF, \textit{etc} à insérer dans tout document
            \item Objet graphique Plotly, un objet Python qui peut directement être affiché dans un Notebook Jupyter.
        \end{itemize}
        \bigbreak
        Les images ne sont pas des graphiques interactifs, mais statiques, concernant le HTML, il l'est, grâce à WebGL, le Python l'est également dans IPython.
        Des sélecteurs peuvent être ajoutés pour filtrer les données, zoomer, \textit{etc}.

        Contrairement à Matplotlib et Seaborn qui ne génèrent que des images uniquement.
        \bigbreak
        L'algorithme qui gère le graphique peut être codé en~:
        \begin{itemize}
            \item Python
            \item JavaScript
            \item R
    ...
    \begin{frame}{Comment choisir la visualisation appropriée~?}{From Data to Viz}
        \url{https://www.data-to-viz.com/}~: \textit{From Data to Viz leads you to the most appropriate graph for your data. It links to the code to build it and lists common caveats you should avoid.}
        \bigbreak
        \centering
        \includegraphics[width=9cm]{image/from-data-to-viz}
    \end{frame}

    \begin{frame}{Plotly}{Plotly et Plotly Express}
        Quasiment tous les types de graphiques sont disponibles, voir \url{https://plotly.com/python/}.
        \bigbreak
        De nombreux langages de programmation sont supportés, y compris ceux plus spécifiques à la data.
        \bigbreak
        L'API se divise en deux parties~:
        \begin{itemize}
            \item \href{https://plotly.com/python/}{Plotly}~: API plus complète, avec de nombreuses options de personnalisation.
            \item \href{https://plotly.com/python/plotly-express/}{Plotly Express}~: pour les graphiques simples, avec une API plus simple et rapide pour prototyper.
        \end{itemize}
        \bigbreak
        \begin{dangercolorbox}
            \emoji{police-car-light} \textbf{Opinion du formateur~:} Il est complètement inutile d'appendre une autre API que celle de Plotly, cette dernière fait tout et mieux\ldots{}
        \end{dangercolorbox}
    \end{frame}

    \subsection{Plotly Express}\label{plotly-express}
    \begin{frame}[fragile]{Plotly}{Plotly et Plotly Express}
        \begin{columns}
            \column{0.5\textwidth}
...
# Exemple de DataFrame avec ventes fictives
df_cars = pd.DataFrame({
    "model": ["Mercedes", "BMW", "Dacia", "Nissan", "Fiat"],
    "price": [41000, 37000, 15000, 15000, 10000],
    "sales": [120, 95, 300, 80, 60]  # Ventes fictives
})

# Camembert des ventes par modèle
fig = px.pie(
    df_cars,
    names="model",
    values="sales",
    title="Répartition des ventes par modèle"
)
fig
            \end{lstlisting}
            \column{0.5\textwidth}
            \centering
            \includegraphics[width=6cm]{image/plotly-pie-chart}
        \end{columns}
        \flushleft
        \bigbreak
        Un graphique minimal peut être généré en passant directement le DataFrame à la fonction \lstinline{plotly.express.pie}.

        Mais même cette version \textquote{simple} offre de nombreuses possibilités de personnalisation, voir \url{https://plotly.com/python-api-reference/generated/plotly.express.pie}.
    \end{frame}

    \begin{frame}{Plotly Express}{Plotly Express et Group By}
        \begin{dangercolorbox}
            Les données sous forme de \lstinline{Series} doivent avoir un nom de colonne et non un nom d'index.
            Hors, le Group By transforme une Series en index par défault.
            Il faut utiliser une méthode qui régénère l'index comme \lstinline{reset_index}.
        \end{dangercolorbox}
        \begin{columns}
            \column{0.5\textwidth}
            \centering
            \includegraphics[width=6cm]{image/groupby-indexed}
            \column{0.5\textwidth}
            \centering
            \includegraphics[width=6cm]{image/groupby-reset-index}
        \end{columns}
    \end{frame}
...
df_cars = pd.DataFrame({
    "model": ["Mercedes", "BMW", "Dacia", "Nissan", "Fiat"],
    "price": [41000, 37000, 15000, 15000, 10000],
    "sales": [120, 95, 300, 80, 60]
})

# Création du graphique en secteurs avec plotly.graph_objects
# On peut avoir plusieurs graphiques dans la même figure
fig = go.Figure(
    data=[go.Pie(labels=df_cars["model"], values=df_cars["sales"])]
)
fig.update_layout(title_text="Répartition des ventes par modèle")
fig
            \end{lstlisting}
            \column{0.5\textwidth}
            \centering
        ...
        \flushleft
        \bigbreak
        Le même graphique peut être généré avec l'API Plotly, \lstinline{plotly.graph_objects}, voir \url{https://plotly.com/python/graph-objects/}.
    \end{frame}

    \subsection{Exercice}\label{plotly-exercice}
    \begin{frame}{Plotly}{Exercice \execcounterdispinc{}}
        Ouvrir le Notebook \url{https://github.com/DigicompClassesByPapIT/PYDATA/blob/main/4_plotly.ipynb}.
        \bigbreak
        \centering
        \includegraphics[width=7cm]{image/digicomp-lightbulb}
    \end{frame}
    ...
    \section{Exemple d'analyse de données en open data sur l'énergie}
    \begin{frame}{Utilisation de l'open data à des fins marketing}{Exercice \execcounterdispinc{}}
        Ouvrir le Notebook \url{https://github.com/DigicompClassesByPapIT/PYDATA/blob/main/5_marketing_minergie.ipynb}.
        \bigbreak
        \centering
        \includegraphics[width=7cm]{image/digicomp-lightbulb}
    \end{frame}
    ...
    \begin{frame}{Introduction au Machine Learning}{Régression linéaire avec Scikit-learn}
        Télécharger et ouvrir le notebook \url{https://github.com/DigicompClassesByPapIT/Python/blob/main/6_simple_linear_regression}.
    ...
    \begin{frame}{Introduction au Machine Learning}{Régression linéaire avec Scikit-learn}
        ...
        \begin{dangercolorbox}
            ...
            Il a besoin des mêmes outils pour le prétraitement des données (A.K.A \textit{data wrangling}), la visualisation, la manipulation, \textit{etc}.
    ...
    \begin{frame}{Deep Learning (réseau de neuronnes récurrents)}{Exercice \execcounterdispinc{}}
        Ouvrir le Notebook \url{https://github.com/DigicompClassesByPapIT/PYDATA/blob/main/7_tensorflow_btc_prediction.ipynb}.
        \bigbreak
        \centering
        \includegraphics[width=12cm]{image/btc-price-prediction}
    \end{frame}
